...
export const step1Schema = z.object({
  ...
  justificativa: z
    .string()
    .min(30, "Justificativa deve ter pelo menos 30 caracteres"),
...
export function saldoEfetivo(
  f: IFornecimento,
  reservaMap: Map<string, number> | null | undefined,
): number {
  const bruto = f.saldoDisponivel ?? f.saldo ?? 0;
  if (!reservaMap) return bruto;
  const reservado = reservaMap.get(f.identificador);
  if (reservado === undefined) return bruto;
...
export function getItemName(item: IItemRequisicao): string {
  ...
  return (
    i?.descricaoBreve ?? i?.descBreve ?? i?.numItem ?? f?.identificador ?? "—"
  );
}
