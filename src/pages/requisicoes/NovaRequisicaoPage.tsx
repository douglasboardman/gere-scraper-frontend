...
export function NovaRequisicaoPage() {
  ...
  const [selectedCompra, setSelectedCompra] = useState<IContratacao | null>(
    null,
  );
  const [selectedItems, setSelectedItems] = useState<
    Map<string, SelectedItemEntry>
  >(new Map());
  const [selectedContratoOob, setSelectedContratoOob] =
    useState<IContratoOob | null>(null);
  const [selectedConfig, setSelectedConfig] =
    useState<IConfigContratacao | null>(null);

  const userUasg =
    typeof user?.unidade === "object"
      ? (user.unidade as IUnidade).uasg
      : undefined;
    ...
    <div>
      <PageHeader
        ...
        actions={
          <Button
            variant="outline"
            onClick={() => navigate("/requisicoes/minhas_requisicoes")}
          >
      ...
      {step === 2 &&
        userUasg &&
        step1Data &&
        step1Data.destDespesa === "Outras_Obrigacoes" && (
      ...
      {step === 2 && (!userUasg || !step1Data) && (
        <Card className="max-w-lg mx-auto">
          <CardContent className="py-10 text-center text-muted-foreground text-sm">
            Não foi possível determinar sua unidade. Tente fazer login
            novamente.
          </CardContent>
        </Card>
      )}
      ...
      {step === 3 && requisicao && selectedCompra && step1Data && (
        <Step3Itens
          ...
          configContratacao={selectedConfig}
          onComplete={(items) => {
